...
public:
	...
	mmuMem(sc_module_name name_, char* codeFileName, char* dataFileName) : sc_module(name_) {
		...
		SC_METHOD(lectura);
		sensitive << regModulo << goodDataOut[0] << goodDataOut[1] << goodDataOut[2] << goodDataOut[3] << flagWhichIO << regIO0 << regIO2 << regLectura;
		SC_METHOD(registrar);
		sensitive << clk.pos();
	}

	~mmuMem() {
		//mem[1]->volcado();  INNECESARIO. SOLO SI SE QUIERE VER EL CONTENIDO FINAL DE LA MEMORIA DE DATOS
		delete mem[0];
		delete mem[1];
		delete mem[2];
		delete mem[3];
...
private:
	...
	sc_signal<sc_uint<3>> regModulo;
	sc_signal < sc_int<32>>	regIO0, regIO2;
	sc_signal <bool> flagWhichIO, regLectura;
