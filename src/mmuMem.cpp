...
void mmuMem::direcciones() {
	...
	nineBaddress	.write(addr		.read().range(10, 2)); 
	writeAddress	.write(extAddrIn.read().range(8, 0));	
	extAddrOut		.write(addr		.read().range(11, 2));
...
void mmuMem::lectura() {		// es b�sicamente un MUX
	...
	case 6: // IO
		if (flagWhichIO.read() == 0)			// no considero una lectura fuera de las direcciones legales
			dataOut.write(regIO0.read());
		else
			dataOut.write(regIO2.read());
		break;
	default:
		dataOut.write(0);
	};
...
void mmuMem::registrar() {	// sensible s�lo al reloj (no hay reset)

	if (rst.read()) {
		regIO0.write(0);		regIO2.write(0);
		regLectura.write(0);	regModulo.write(0);		flagWhichIO.write(0);
	}
	else {

		if (regLectura.read() && (regModulo.read() == 6)) { // resetea el registro IO despu�s de ser le�do
		...
		int a = addr.read();
		int b = R.read();
		flagWhichIO.write(addr.read().bit(3));	// supongo que no se intentan direcciones "ilegales"
	}
}
...
int  mmuMem::registro(char* cad) {
	...
	int i;
	if (cad[0] = !'$') {
		fprintf(stderr, "registro inv�lido %s\n", cad);
		exit(-1);
	}
	cad++;
	for (i = 0; i < 32; ++i) {
...
void mmuMem::parseProgram() {
	...
	if ((programa[NL][0] >= '0') && (programa[NL][0] <= '9')) { // suponemos que le programa ya est� ensamblado, lo leemos en memoria y nos vamos
		i = 0;
		cad = programa[NL];
		if ((cad[0] == '0') && ((cad[1] == 'x') || (cad[1] == 'X')))
			sscanf(cad, "%x", &ent);
	...
	for (i = 0; i < NL; ++i) {
		...
		if (((inst >= 10) && (inst <= 16)) || (inst == 19)) {	// inm de instrucciones aritm�tico l�gica incluidos desplazamientos y lui
			pt = parse(buffer, pt);
			inm = parseInm(buffer);
		}
		...
		if (inst == 25) {	// mflo en el �ltimo momento
			pt = parse(buffer, pt);
			reg1 = registro(buffer);
		}
