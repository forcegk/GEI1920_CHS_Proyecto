...
void MIPScore::codigoFiltrar() {
	...
	esperaPosterior:
		...
		LW($t0, 0x3000 + 16);		// lee la "m�scara" para saber qu� frecuencias pasan y cuales no
		...
		if( !(Rt0 & 0x01) )	{	// si el bit '0' de la m�scara es 0, reduzco el peso de esta frecuencia en un factor 32
			Rt1 >>= 5;
			Rt2 >>= 5;
		}
		...
		if( !(Rt0 & 0x02) )	{	// si el bit '1' de la m�scara es 0, reduzco el peso de esta frecuencia en un factor 32
			Rt1 >>= 5;
			Rt2 >>= 5;
		}
		SW($t1, Ra1 + (i << 3));		// almacena la parte real
		...
		for(i=2; i<4; ++i){
			...
			if( !(Rt0 & 0x04) )	{	// si el bit '2' de la m�scara es 0, reduzco el peso de estas frecuencias en un factor 32
				Rt1 >>= 5;
				Rt2 >>= 5;
			}
			SW($t1, Ra1 + (i << 3));		// almacena la parte real
		...
		for(i=4; i<8; ++i){
			...
			if( !(Rt0 & 0x08) )	{	// si el bit '3' de la m�scara es 0, reduzco el peso de estas frecuencias en un factor 32
				Rt1 >>= 5;
				Rt2 >>= 5;
			}
			SW($t1, Ra1 + (i << 3));		// almacena la parte real
		...
		for(i=8; i<16; ++i){
			...
			if( !(Rt0 & 0x10) )	{	// si el bit '4' de la m�scara es 0, reduzco el peso de estas frecuencias en un factor 32
				Rt1 >>= 5;
				Rt2 >>= 5;
			}
			SW($t1, Ra1 + (i << 3));		// almacena la parte real
		...
		for(i=16; i<32; ++i){
			...
			if( !(Rt0 & 0x20) )	{	// si el bit '5' de la m�scara es 0, reduzco el peso de estas frecuencias en un factor 32
				Rt1 >>= 5;
				Rt2 >>= 5;
			}
			SW($t1, Ra1 + (i << 3));		// almacena la parte real
		...
		for(i=32; i<64; ++i){
			...
			if( !(Rt0 & 0x40) )	{	// si el bit '6' de la m�scara es 0, reduzco el peso de estas frecuencias en un factor 32
				Rt1 >>= 5;
				Rt2 >>= 5;
			}
			SW($t1, Ra1 + (i << 3));		// almacena la parte real
		...
		// frecuencias 64 a 127		(incluida parte "reflejada", por eso sigue con 64 m�s)
		for(i=64; i<192; ++i){
			LW($t1, Ra0 + (i << 3));		// carga la parte real
			LW($t2, Ra0 + (i << 3) + 4);	// carga la parte img

			if( !(Rt0 & 0x80) )	{	// si el bit '7' de la m�scara es 0, reduzco el peso de estas frecuencias en un factor 32
				Rt1 >>= 5;
				Rt2 >>= 5;
			}
			SW($t1, Ra1 + (i << 3));		// almacena la parte real
		...
		// ahora se env�an las se�ales de sincronismo y se vuelve a empezar
